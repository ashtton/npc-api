...
public class NMSFakePlayer extends NPCFakePlayer {
    ...
    public void rotate(Player player, float yaw, float pitch) {
        ...
        connection.sendPacket(new PacketPlayOutEntityLook(entityPlayer.getId(), (byte) ((int) (yaw * 256.0F / 360.0F)), (byte) ((int) (pitch * 256.0F / 360.0F)), true));
    }
    ...
    public void show(Player player) {
        PlayerConnection connection = ((CraftPlayer) player).getHandle().playerConnection;
        connection.sendPacket(PacketPlayOutPlayerInfo.addPlayer(entityPlayer));
        ...
        new Thread() {
            ...
            public void run() {
                Thread.sleep(1000);
                connection.sendPacket(PacketPlayOutPlayerInfo.removePlayer(entityPlayer));
            }
        }.start();
    }
